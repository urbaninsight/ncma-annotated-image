...
function ncma_annotated_image_clone_post_link( $actions, $post ) {
	...
	if ( 
		$post->post_type === 'ncma-annotated-image'
	) {
		...
		$url = wp_nonce_url(
			add_query_arg(
				array(
					'action' => 'ncma_annotated_image_clone_post_as_draft',
					'post' => $post->ID,
				),
				'admin.php'
			),
		...
		$actions[ 'clone' ] = '<a href="' . $url . '" title="Clone this item to a new draft" rel="permalink">Clone</a>';
	}
...
/*
 * Function creates post clone as a draft and redirects then to the edit post screen
 */
add_action( 'admin_action_ncma_annotated_image_clone_post_as_draft', 'ncma_annotated_image_clone_post_as_draft' );

function ncma_annotated_image_clone_post_as_draft(){

	// check if post ID has been provided and action
	if ( empty( $_GET[ 'post' ] ) ) {
		wp_die( 'No post to clone has been provided!' );
	}
	...
	// if post data exists (I am sure it is, but just in a case), create the post clone
	if ( $post ) {
 ...
 */
add_action( 'admin_notices', 'ncma_annotated_image_duplication_admin_notice' );

function ncma_annotated_image_duplication_admin_notice() {
